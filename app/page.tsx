...
export default function Home() {
  ...
  return (
    <main className="min-h-screen p-8 max-w-3xl mx-auto">
      <h1 className="text-4xl font-bold mb-2 text-center">Crossword Clue Lookup</h1>
      <p className="text-gray-400 text-center mb-8">输入单词，查找填字游戏线索</p>
      ...
      <form onSubmit={handleSearch} className="mb-8">
        <div className="flex gap-3">
          ...
          />
          <button type="submit" className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition">
            查找
          </button>
        </div>
      </form>
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
          ...
          <div className="space-y-2">
            {result.word.clues.map((clue: string, i: number) => (
              <div key={i} className="flex items-center justify-between bg-gray-700 px-4 py-2 rounded">
                <span>{clue}</span>
                <button onClick={() => copyToClipboard(clue)} className="text-gray-400 hover:text-white">📋</button>
              </div>
            ))}
          </div>
